        ...
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap">
              ...
              <!--通过v-show来控制显不显示，v-show是隐藏，v-if是直接把标签干掉-->
              <!--errorTip是true就显示，是false就不显示-->
            </div>
            <ul>
...
<script>
  import './../assets/css/login.css'  //  导入样式
  ...
  export default {
    ...
    data() {
      return {
        userName: '',   //  定义登录账号字段
        userPwd: '',  //  定义密码字段
        errorTip: false   //  定义错误提示字段，校验提示字段，默认提示
      }
    },
    methods: {
      login() {  //  定义登录函数
        var a = this;
        axios.post("/apis/api-auth/login/",   //  登录使用post
          {
            username: a.userName,
            password: a.userPwd
          }, {
            headers: {
              'token': '123',
              'X-CSRFToken': a.getCookie('csrftoken')
            }
          }
        ).then((response) => {
          let res = response.data;
          if (response.status == 200) {
            a.errorTip = false;   //  如果响应成功，就不弹出错误提示
            // to-do.等待后续
          } else {
            a.errorTip = true;  //  如果响应失败，就弹出错误提示
          }
        })
      },
